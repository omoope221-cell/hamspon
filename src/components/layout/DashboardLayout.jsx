...
import { roleDisplayName, loginPathFor } from '../../utils/roles';
...
export default function DashboardLayout() {
  ...
  // The dashboard content area scrolls independently of the window, so
  // switching pages needs its own reset in addition to the app-wide
  // <ScrollToTop /> (which only tracks window scroll).
  useEffect(() => {
  ...
  async function handleLogout() {
    const dest = loginPathFor(user?.accountType);
    await logout();
    ...
    <div className="brand-theme min-h-screen flex bg-[var(--paper-100)]">
      ...
      >
        <div className="flex items-center gap-3 px-5 py-5 bg-blue-600">
          <Crest size={32} />
          <div>
            <p className="font-display text-sm font-semibold leading-tight text-white">{settings.schoolName}</p>
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors ${
                  isActive
                    ...
                    : 'text-[var(--slate-600)] hover:text-pink-500 hover:bg-pink-50'
                }`
              }
            >
              <Icon size={16} />
